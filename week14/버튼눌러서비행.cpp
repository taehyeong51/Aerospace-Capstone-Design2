...
float stabilize_pterm_roll,stabilize_iterm_roll=0,desired_rate_roll=0,rate_error_roll,rate_pterm_roll,rate_iterm_roll;
float stabilize_pterm_pitch,stabilize_iterm_pitch=0,desired_rate_pitch=0,rate_error_pitch,rate_pterm_pitch,rate_iterm_pitch;
// --------------------------------------------------------
...
void RF_READ()
{
    if (NRF24L01.available())
    {
        ...
        if(BUT2 == 0){rate_kp = 0, rate_ki = 0,rate_kd=0, Roll_pid_in=0,Pitch_pid_in=0,stabilize_iterm_roll=0,desired_rate_roll=0,
                                            stabilize_iterm_pitch=0,desired_rate_pitch=0,stabilize_kp=0,
                                            rate_iterm_pitch = 0, rate_iterm_pitch = 0;}
        // if(throttle_offset < 10){
        //     if(BUT1 == 1 && BUT2 == 0){rate_ki += 0.001;}
        //     else if(BUT2 == 1 && BUT1 == 0){rate_kp += 0.01;}
        //     else if(BUT1 == 0 && BUT2 == 0){rate_kp = 0, rate_ki = 0,rate_kd=0, Roll_pid_in=0,Pitch_pid_in=0,stabilize_iterm_roll=0,desired_rate_roll=0,
        //                                     stabilize_iterm_pitch=0,desired_rate_pitch=0,stabilize_kp=0,
        //                                     rate_iterm_pitch = 0, rate_iterm_pitch = 0;}
        // }
        // else{
        //     if(BUT2 == 0 && BUT1 == 1){rate_kd += 0.001;}
        //     else if(BUT2 == 1 && BUT1 == 0){stabilize_kp += 0.01;}
        //     else if(BUT1 == 0 && BUT2 == 0){rate_kp = 0, rate_ki = 0,rate_kd=0, Roll_pid_in=0,Pitch_pid_in=0,stabilize_iterm_roll=0,desired_rate_roll=0,
        //                                     stabilize_iterm_pitch=0,desired_rate_pitch=0,stabilize_kp=0,
        //                                     rate_iterm_pitch = 0, rate_iterm_pitch = 0;}
        }
...
float stabilize_dterm_roll,rate_dterm_roll,prev_rate_roll=0;

float stabilize_dterm_pitch,rate_dterm_pitch,prev_rate_pitch=0;

void dual_pid_roll(float target,float current,float rate) {
    
    error_roll = target - current;
    
    stabilize_pterm_roll = stabilize_kp * error_roll;
    // stabilize_iterm_roll += stabilize_ki * error_roll * dt;

    desired_rate_roll = stabilize_pterm_roll;

    rate_error_roll = desired_rate_roll - rate;

    rate_pterm_roll = rate_kp * rate_error_roll;
    rate_iterm_roll += rate_ki * rate_error_roll * dt;
    rate_dterm_roll = -rate_kd * (prev_rate_roll-rate) / dt;
    prev_rate_roll = rate;
    Roll_pid_in = (stabilize_pterm_roll+rate_pterm_roll + rate_iterm_roll + stabilize_iterm_roll)/360*100;
}



void dual_pid_pitch(float target,float current,float rate) {
    
    error_pitch = target - current;
    
    stabilize_pterm_pitch = stabilize_kp * error_pitch;
    stabilize_iterm_pitch += stabilize_ki * error_pitch * dt;

    desired_rate_pitch = stabilize_pterm_pitch;

    rate_error_pitch = desired_rate_pitch - rate;

    rate_pterm_pitch = rate_kp * rate_error_pitch;
    rate_iterm_pitch += rate_ki * rate_error_pitch * dt;
    rate_dterm_roll = -rate_kd * (prev_rate_pitch-rate) / dt;

    prev_rate_pitch = rate;

    Pitch_pid_in = (stabilize_pterm_pitch+rate_pterm_pitch + rate_iterm_pitch + stabilize_iterm_pitch)/360*100;
}
...
void activate(){
    ...
    // Doual PID
    dual_pid_roll(target_roll,roll_c,gyro_f[0]);
    dual_pid_pitch(target_pitch,pitch_c,gyro_f[1]);
    ...
    // if (cnt > 600){
    //     // pc.printf("\r\ntarget : %2.2f current ; %2.2f",target_roll,roll_c);
    //     // pc.printf("\r\nError Roll : %3.2f, Angular Rate : %3.2f, Roll PID In : %3.2f",error_roll,gyro_f[0],Roll_pid_in);
    ...
    //     pc.printf("\rKp1 : %2.3f Kp2 : %2.3f Ki : %2.3f Kd : %2.3f",stabilize_kp,rate_kp,rate_ki,rate_kd);
    // }
...
int main() {
    ...
    while (BUT1 == 1){
        pc.pritnf("\r BUT1 not clicked")
        wait(1);
    }
    pc.printf("\r BUT1 clicked");
    ...
    pc.printf("\r\ntarget roll : %f, target pitch : %f, tmp_bias_roll : %f, tmp_bias_pitch : %f" ,target_roll,target_pitch,tmp_bias_roll,tmp_bias_pitch);
